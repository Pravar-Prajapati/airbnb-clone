...
export const NavigationHeader: React.FC<NavigationHeaderProps> = ({
  ...
  const isVisible = showSubBar !== undefined ? showSubBar : isScrolledPastHero;
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-zinc-200 transition-all">
      ...
      {/* Sticky Sub-Bar (rendered on scroll past hero section) */}
      {isVisible && (
        <div
          data-testid="navigation-sub-bar"
          className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-zinc-200 shadow-md transition-all duration-200"
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
            ...
            {/* Right Side: Price, Rating Summary & Solid Magenta Reserve Button */}
            <div className="flex items-center gap-4 sm:gap-6">
              ...
              <button
                ...
                className="bg-[#E51D53] hover:bg-[#D70466] text-white font-semibold rounded-lg px-6 py-3 transition-colors active:scale-95 text-sm sm:text-base whitespace-nowrap cursor-pointer shadow-sm"
              >
                Reserve
              </button>
            </div>
